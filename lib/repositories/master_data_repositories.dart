...
class OrganizationRepository {
  ...
  Future<List<Organization>> getOrganizations() async {
    final data = await _client.from('organizations').select().order('code');
  ...
  Future<Organization> createOrganization(Organization org) async {
    ...
    final data = await _client.from('organizations').insert(json).select().single();
...
class EmployeeRepository {
  ...
  Future<List<Employee>> getEmployees() async {
    final data = await _client.from('employees').select().order('full_name');
  ...
  Future<Employee> createEmployee(Employee employee) async {
    ...
    final data = await _client.from('employees').insert(json).select().single();
...
class BudgetRepository {
  ...
  Stream<List<Budget>> getBudgetsStream({int? fiscalYear}) {
    var query = _client.from('budgets').stream(primaryKey: ['id']);
  ...
  Future<List<Budget>> getBudgetsByYear(int year) async {
     final data = await _client.from('budgets').select().eq('fiscal_year', year);
  ...
  Future<Budget> createBudget(Budget budget) async {
    ...
    final data = await _client.from('budgets').insert(json).select().single();
...
class VendorRepository {
  ...
  Future<List<Vendor>> getVendors() async {
    final data = await _client.from('vendors').select().order('name');
  ...
  Future<Vendor> createVendor(Vendor vendor) async {
    ...
    final data = await _client.from('vendors').insert(json).select().single();
    return Vendor.fromJson(data);
  }

  Future<void> updateVendor(Vendor vendor) async {
    await _client.from('vendors').update(vendor.toJson()).eq('id', vendor.id);
  }
...
class AssetCategoryRepository {
  ...
  Future<List<AssetCategory>> getCategories() async {
    final data = await _client.from('asset_categories').select().order('code');
  ...
  Future<AssetCategory> createCategory(AssetCategory category) async {
    ...
    final data = await _client.from('asset_categories').insert(json).select().single();
    return AssetCategory.fromJson(data);
  }

  Future<void> updateCategory(AssetCategory category) async {
    await _client.from('asset_categories').update(category.toJson()).eq('id', category.id);
  }

  Future<void> deleteCategory(String id) async {
    await _client.from('asset_categories').delete().eq('id', id);
  }
}
...
class AssetRepository {
  ...
  Future<List<MasterAset>> searchAssets(String query) async {
    final queryBuilder = _client.from('assets')
      .select('id, asset_code, name, asset_categories(name), condition, location_id, image_url'); 
    ...
    return (data as List).map((e) => MasterAset.fromJson({
      ...
      'name': e['name'],
      'category': e['asset_categories'] != null ? e['asset_categories']['name'] : 'Umum', 
  ...
  /// Returns full Asset model to provide all details (price, purchase date, etc.)
  Future<List<Asset>> getAssetsForReport({
    required DateTime startDate,
    required DateTime endDate,
    String? category, // 'A', 'B', 'C', or 'all'
  }) async {
    // Select all columns to fully populate Asset model from assets
    // Note: Assuming 'locations' relationship exists on assets. 
    // If not, locationName might be null, which is handled by model.
    var query = _client.from('assets')
      .select('*, locations:locations(name), asset_categories(name)') 
      .gte('created_at', startDate.toIso8601String())
      .lte('created_at', endDate.toIso8601String())
      .neq('status', 'disposed'); // Exclude disposed assets

    // Apply Category Filter logic (KIB A, B, etc.)
    // Note: 'assets' usually relies on 'category_id' or 'category' string.
    // If filtering by string 'category' field:
    if (category != null && category != 'all') {
        // Logic KIB mapping (Disabled due to column refactor - TODO: Fix with proper ID filtering)
      /*
      if (category == 'A') { // Tanah
        ...
        // Note: Supabase filter chaining is AND by default
      ...
      */
      // Temporary fallback: return all, filter in client or by category_id if known across env
      // query = query.filter('asset_categories.code', 'eq', category); // If we knew codes
      // Add more as needed
    }

    final data = await query.order('created_at');

    return (data as List).map((e) => Asset.fromSupabase(e)).toList();
  }
}
