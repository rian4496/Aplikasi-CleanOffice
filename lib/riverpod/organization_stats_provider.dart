...
final organizationStatsProvider = FutureProvider<Map<String, OrganizationStats>>((ref) async {
  ...
  // 4. Aggregate Asset Counts (now using organizationId)
  final assetCounts = <String, int>{};
  for (var asset in (assets as List)) {
    final orgId = (asset as dynamic).organizationId;
    if (orgId != null && statsMap.containsKey(orgId)) {
      assetCounts[orgId] = (assetCounts[orgId] ?? 0) + 1;
    }
  }
