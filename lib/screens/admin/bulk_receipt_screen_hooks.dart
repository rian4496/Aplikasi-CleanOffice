// lib/screens/admin/bulk_receipt_screen_hooks.dart
// ✅ MIGRATED TO HOOKS_RIVERPOD
// Bulk receipt generation from Excel file
...
// 4. UI
/// Bulk Receipt Screen - Generate receipts from Excel file
/// ✅ MIGRATED: ConsumerWidget → HookConsumerWidget
class BulkReceiptScreen extends HookConsumerWidget {
  ...
  /// Build status widget based on current state
  static Widget _buildStatusWidget(BuildContext context, BulkReceiptState state) {
  ...
  /// Build button widget based on current state
  static Widget _buildButtonWidget(
      BuildContext context, BulkReceiptState state, BulkReceiptNotifier notifier) {
    ...
    if (state.status == BulkReceiptStatus.success) {
      return ElevatedButton.icon(
        ...
        onPressed: state.downloadUrl != null
            ? () => _downloadFile(context, state.downloadUrl!)
            : null,
      );
    }
  ...
  static Future<void> _downloadFile(BuildContext context, String url) async {
