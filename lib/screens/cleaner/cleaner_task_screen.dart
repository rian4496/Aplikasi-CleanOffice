...
class CleanerTaskScreen extends ConsumerWidget {
  const CleanerTaskScreen({super.key});
      ...
      body: ticketsAsync.when(
        data: (tickets) {
          ...
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(cleanerTasksProvider),
...
class _TaskCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return InkWell(
      onTap: () => context.push('/cleaner/task/${ticket.id}'),
          ...
          Row(
            ...
            children: [
              if (isClaimed) ...[
                ...
                ElevatedButton.icon(
                  onPressed: () => context.push('/cleaner/task/${ticket.id}'),
  ...
  Future<void> _updateStatus(BuildContext context, WidgetRef ref, TicketStatus newStatus) async {
    try {
      ...
      ref.invalidate(cleanerTasksProvider); // Refresh list
