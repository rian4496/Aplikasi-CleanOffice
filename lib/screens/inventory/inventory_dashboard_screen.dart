...
class _InventoryDashboardScreenState
  ...
  Widget _buildDesktopActionsGrid(BuildContext context, bool isAdmin) {
  ...
  Widget _buildMobileActionsGrid(BuildContext context, bool isAdmin) {
    final actions = _getActionsList(context, isAdmin);
    ...
    return GridView.count(
      ...
      children: actions.map((action) => _buildCompactActionCard(
        icon: action['icon'] as IconData,
        label: action['label'] as String,
        color: action['color'] as Color,
        onPressed: action['onPressed'] as VoidCallback,
  ...
  List<Map<String, dynamic>> _getActionsList(BuildContext context, bool isAdmin) {
  ...
  // ==================== LOW STOCK ALERT ====================
  Widget _buildLowStockAlert(BuildContext context, List<InventoryItem> items) {
    final lowStockItems = items.where((i) =>
      i.status == StockStatus.lowStock || i.status == StockStatus.outOfStock
    ).toList();
  ...
  // ==================== CATEGORY BREAKDOWN ====================
  Widget _buildCategoryBreakdown(BuildContext context, List<InventoryItem> items, bool isDesktop) {
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final categoryItems = items.where((i) => i.category == category.name).toList();
              final totalStock = categoryItems.fold(0, (sum, item) => sum + item.currentStock);
