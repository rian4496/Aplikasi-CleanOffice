// lib/screens/teknisi/teknisi_inbox_screen.dart
...
import 'package:aplikasi_cleanoffice/riverpod/teknisi_providers.dart';
import '../../riverpod/auth_providers.dart';
import '../../widgets/shared/empty_state_widget.dart';

enum SortOption { latest, oldest, urgent }

class TeknisiInboxScreen extends ConsumerStatefulWidget {
  const TeknisiInboxScreen({super.key});

  @override
  ConsumerState<TeknisiInboxScreen> createState() => _TeknisiInboxScreenState();
}

class _TeknisiInboxScreenState extends ConsumerState<TeknisiInboxScreen> {
  ...
  Widget build(BuildContext context) {
    final ticketsAsync = ref.watch(teknisiInboxProvider);
                              ...
                              Positioned.fill(
                                child: _searchQuery.isNotEmpty 
                                  ...
                                  : EmptyStateWidget.noTasks(
                                      onCreateTask: () => ref.invalidate(teknisiInboxProvider),
                                    ),
                              ),
                            ],
                          )
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          ElevatedButton(
            onPressed: () => ref.invalidate(teknisiInboxProvider),
  ...
  Future<void> _handleClaimTicket() async {
    ...
    try {
      ...
      if (userId != null) {
        ...
        widget.ref.invalidate(teknisiInboxProvider);
        widget.ref.invalidate(teknisiTasksProvider); // Refresh "Tugas Saya"
        widget.ref.invalidate(teknisiTicketStatsProvider); // Refresh stats
        if (mounted) {
