...
class TeknisiScheduleScreen extends HookConsumerWidget {
  const TeknisiScheduleScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final activeReportsAsync = ref.watch(teknisiTasksProvider);
    ...
    return Scaffold(
      ...
      body: activeReportsAsync.when(
        data: (reports) {
          ...
          final sortedReports = List<Ticket>.from(reports)
          ...
          for (var report in sortedReports) {
            final dateKey = DateFormat('EEEE, d MMM yyyy', 'id_ID').format(report.createdAt); // Requires locale setup or just d MMM yyyy
            // Fallback unique key
            final key =  DateFormat('d MMMM yyyy').format(report.createdAt);
  ...
  Widget _buildScheduleCard(BuildContext context, Ticket report) {
    final isUrgent = report.priority == TicketPriority.urgent;
    return Container(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        report.locationName ?? 'Lokasi tidak diketahui',
