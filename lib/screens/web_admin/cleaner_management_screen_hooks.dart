// lib/screens/web_admin/cleaner_management_screen_hooks.dart
...
class CleanerManagementScreen extends HookConsumerWidget {
  const CleanerManagementScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // ✅ HOOKS: State management
    final sortBy = useState<String>('name'); // 'name', 'tasks', 'performance'
    ...
    return Scaffold(
      ...
      // ==================== APP BAR (Mobile Only) ====================
      appBar: !isDesktop ? _buildMobileAppBar(sortBy) : null,
      ...
      // ==================== BODY ====================
      body: isDesktop
          ? _buildDesktopLayout(context, ref, cleanersAsync, sortBy)
          : _buildMobileLayout(context, ref, cleanersAsync, sortBy),
    );
  }
  ...
  static AppBar _buildMobileAppBar(ValueNotifier<String> sortBy) {
    return AppBar(
      title: const Text(
        'Kelola Petugas',
              ...
              child: Row(
                children: [
                  Icon(
                    Icons.sort_by_alpha,
                    size: 20,
                    color: sortBy.value == 'name' ? AppTheme.primary : Colors.grey,
                  ),
              ...
              child: Row(
                children: [
                  Icon(
                    Icons.assignment,
                    size: 20,
                    color: sortBy.value == 'tasks' ? AppTheme.primary : Colors.grey,
                  ),
                  const SizedBox(width: 12),
              ...
              child: Row(
                children: [
                  Icon(
                    Icons.trending_up,
                    size: 20,
                    color: sortBy.value == 'performance' ? AppTheme.primary : Colors.grey,
                  ),
                  const SizedBox(width: 12),
                  const Text('Performa'),
                ],
  ...
  static Widget _buildDesktopLayout(
    ...
    WidgetRef ref,
    AsyncValue<List<CleanerProfile>> cleanersAsync,
              ...
              Expanded(
                child: cleanersAsync.when(
                  data: (cleaners) {
                    ...
                    // Sort cleaners
                    var sortedCleaners = _sortCleaners(cleaners, sortBy.value);
                          ...
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return _buildCleanerCard(context, cleaner, index);
                              },
                            ),
                          ),
                        ],
  ...
  /// Sort cleaners based on criteria
  static List<CleanerProfile> _sortCleaners(
    List<CleanerProfile> cleaners,
    String sortBy,
  ) {
    ...
    switch (sortBy) {
      case 'name':
        ...
        break;
      case 'tasks':
        sortedCleaners.sort((a, b) => b.activeTaskCount.compareTo(a.activeTaskCount));
        break;
      case 'performance':
  ...
  /// Build desktop header (blue bar)
  static Widget _buildDesktopHeader(ValueNotifier<String> sortBy) {
    return Container(
      height: 70,
  ...
  static Widget _buildSummaryStats(List<CleanerProfile> cleaners) {
    final totalCleaners = cleaners.length;
    final activeCleaners = cleaners.where((c) => c.activeTaskCount > 0).length;
    final availableCleaners = cleaners.where((c) => c.activeTaskCount == 0).length;
    final totalTasks = cleaners.fold<int>(0, (sum, c) => sum + c.activeTaskCount);
        ...
        children: [
          Row(
            children: [
              ...
              _buildStatItem('Aktif', activeCleaners, Icons.work, AppTheme.info),
              _buildStatItem(
                  'Tersedia', availableCleaners, Icons.check_circle, AppTheme.success),
            ],
          ),
  ...
  /// Build stat item
  static Widget _buildStatItem(String label, int value, IconData icon, Color color) {
    return Expanded(
      child: Column(
        children: [
  ...
  /// Build cleaner card
  static Widget _buildCleanerCard(
    BuildContext context,
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: Card(
        ...
        child: InkWell(
          onTap: () => _showCleanerDetail(context, cleaner),
          borderRadius: BorderRadius.circular(12),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        cleaner.activeTaskCount == 0 ? Icons.check_circle : Icons.work,
                        size: 16,
                        color: cleaner.activeTaskCount == 0
                            ? AppTheme.success
                            : AppTheme.warning,
  ...
  /// Build workload indicator
  static Widget _buildWorkloadIndicator(int taskCount) {
    Color color;
    String label;
  ...
  /// Show cleaner detail modal
  static void _showCleanerDetail(BuildContext context, CleanerProfile cleaner) {
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage:
                        cleaner.photoUrl != null ? NetworkImage(cleaner.photoUrl!) : null,
                    child: cleaner.photoUrl == null
