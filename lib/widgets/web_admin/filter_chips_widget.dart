...
// Notifier for QuickFilter state (Riverpod 3 compatible)
class QuickFilterNotifier extends Notifier<QuickFilter> {
  @override
  QuickFilter build() => QuickFilter.all;
  
  void setFilter(QuickFilter filter) => state = filter;
}

final quickFilterProvider = NotifierProvider<QuickFilterNotifier, QuickFilter>(
...
class FilterChips extends ConsumerWidget {
  ...
  Widget _buildChip(
    ...
    return FilterChip(
      ...
      selected: isSelected,
      onSelected: (selected) {
        ref.read(quickFilterProvider.notifier).setFilter(filter);
